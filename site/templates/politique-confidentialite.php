...
$resolveText = static function (string $value, string $contactEmail): string {
  return str_replace(
    ['[E-mail à compléter]', '[e-mail à compléter]'],
    $contactEmail,
    $value
  );
};
  ...
  <section class="legal-content">
    <div class="wrap">
      <article class="legal-card">
        ...
        <p><?= $toHtml($resolveText($text('section1Body', "Le responsable du traitement est :\n\n[Nom et prénom à compléter] EI\n" . $siteTitle . "\nContact : [E-mail à compléter]"), $contactEmail)) ?></p>
      </article>
      ...
      <article class="legal-card">
        ...
        <p><?= $toHtml($resolveText($text('section3Body', "Les données transmises sont utilisées uniquement pour :\n- répondre à une demande\n- préparer un échange ou un rendez-vous\n- assurer le suivi d'une relation commerciale si elle est engagée"), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">4. Base légale</h2>
        <p><?= $toHtml($resolveText($text('section4Body', "Le traitement repose sur :\n- l'exécution de mesures précontractuelles lorsque vous demandez un échange ou un devis\n- l'intérêt légitime de " . $siteTitle . " à répondre aux demandes reçues"), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">5. Destinataires</h2>
        <p><?= $toHtml($resolveText($text('section5Body', "Les données sont destinées uniquement à " . $siteTitle . ".\n\nElles ne sont pas vendues.\nElles ne sont pas transmises à des tiers à des fins publicitaires."), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">6. Durée de conservation</h2>
        <p><?= $toHtml($resolveText($text('section6Body', "Les données issues d'un échange sont conservées pendant la durée nécessaire au traitement de la demande et au suivi de la relation commerciale.\n\nEn l'absence de suite donnée, elles peuvent être supprimées à l'issue d'un délai raisonnable."), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">7. Droits des personnes</h2>
        <p><?= $toHtml($resolveText($text('section7Body', "Vous pouvez demander l'accès, la rectification ou la suppression de vos données.\n\nVous pouvez également vous opposer à leur traitement lorsque cela est applicable.\n\nPour exercer vos droits : [E-mail à compléter]\n\nVous pouvez également introduire une réclamation auprès de la CNIL."), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">8. Cookies et traceurs</h2>
        <p><?= $toHtml($resolveText($text('section8Body', "Le site ne dépose actuellement pas de cookies de mesure d'audience, publicitaires ou de suivi.\n\nAucune bannière cookies n'est donc affichée à ce stade.\n\nSi des outils de mesure d'audience ou traceurs sont ajoutés ultérieurement, la présente politique sera mise à jour."), $contactEmail)) ?></p>
      </article>

      <article class="legal-card">
        <h2 class="serif">9. Données techniques</h2>
        <p><?= $toHtml($resolveText($text('section9Body', "L'hébergeur du site peut traiter certaines données techniques nécessaires au fonctionnement et à la sécurité du site, notamment des journaux de connexion."), $contactEmail)) ?></p>
      </article>
    </div>
  </section>
</main>
